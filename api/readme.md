...
En tant que développeur vous avez la tâche de créer des outils permettant de voter des propositions de loi permettant aux citoyens de sortir de cette crise.
...
- Le projet doit être en mesure de compiler.
...
- Le temps d'accès à la résponse d'un call API est rapide.
- Les temps de réponse des enpoints sont stable.
- Vous consommer très peut de RAM.
...
- Les champs Titre et description sont obligatoire.

#### Édition d'un Vote

Endpoint : PUT /votes/[:uuid]

- Seul un administrateur peut changer les champs Title, Description, StartDate et EndDate.
- Attention l'administrateur n'a pas les droits pour changer la collection d'UUIDVote.
...
- Un votant ne peut voter qu'une fois sur une proposition de vote.
...
ID (int), UUID (string), Title (string), Description (text), UUIDVote (collection), StartDate (time.Time), EndDate (time.Time), CreatedAt (time.Time), UpdatedAt (time.Time), DeletedAt (*time.Time).
